...
const AboutSection = () => {
  ...
  const sectionRef = useRef<HTMLElement>(null); // ✅ AJOUT
  ...
  // ✅ AUTOPLAY / AUTOPAUSE SELON VISIBILITÉ
  useEffect(() => {
  ...
  const goToBlog = () => {
   ...
   return (
    <section
      ref={sectionRef}  
      id="a-propos"
      className="relative py-20 md:py-28 bg-gradient-to-br from-white via-gray-50 to-white overflow-hidden"
    >
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-stretch">
            ...
            >
              ...
              >
                ...
                {/* Lien vers le blog - Intégré discrètement */}
                <motion.div 
                ...
                >
                  <motion.button
                    onClick={goToBlog}
                    className="inline-flex items-center gap-2 px-5 py-2.5 text-gray-600 hover:text-primary font-medium rounded-lg border border-gray-200 hover:border-primary/30 bg-white hover:bg-primary/5 transition-all duration-300 group"
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <BookOpen className="w-4 h-4 group-hover:animate-pulse" />
                    <span>Découvrir nos conseils experts</span>
                    <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </motion.button>
                  <p className="text-gray-400 text-xs mt-3">
                    Accédez à notre blog pour des astuces marketing et des études de cas
                  </p>
                </motion.div>
              </motion.div>
            </motion.div>
