...
import { ArrowLeft, Save, Upload, Eye, X, Image as ImageIcon, FileText, Bold, Italic, Link as LinkIcon, CheckCircle, AlertCircle } from 'lucide-react';
import Link from 'next/link';

// ✅ URL corrigée avec ?action=create
...
// ── ImageInput — URL + Upload (les deux dans le même fichier) ────────────────
...
export default function NewPostPage() {
  ...
  const [form, setForm] = useState<PostFormData>({
    title:'', slug:'', content:'# Nouvel article\n\nCommencez à écrire ici...',
    excerpt:'', category:'Marketing', author:'', is_featured:false,
    tags:[], image_url:'',
  });

  const categories = [
    'Marketing','SEO','Design','Content','Développement','Web','Social Media','IA',
  ];
  ...
  const update = (e: React.ChangeEvent<HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement>) => {
    ...
    setForm(prev => {
      ...
      if (name === 'title') next.slug = makeSlug(value);
      return next;
    });
  };

  // ✅ Envoi vers ?action=create (POST) ou ?action=update&id=X (PUT)
  const submit = async (publish: boolean) => {
    if (!form.title.trim()) { showToast('error','Le titre est obligatoire'); return; }
    if (!form.content.trim() || form.content === '# Nouvel article\n\nCommencez à écrire ici...') {
      showToast('error','Le contenu est obligatoire'); return;
    }
    ...
    try {
      ...
      // ✅ URL avec ?action=create — compatible avec blog.php POO
      ...
      try {
        ...
        throw new Error('Le serveur a retourné une erreur PHP. Vérifiez les logs XAMPP.');
      }

      if (data.success) {
        showToast('success', publish ? 'Article publié !' : 'Brouillon enregistré !');
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5">Blog VISIOAD</p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button type="button" onClick={() => submit(false)} disabled={submitting}
                className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 bg-white rounded-xl hover:border-gray-300 transition text-sm font-medium text-gray-700 disabled:opacity-50">
            ...
            <div className={preview ? 'lg:col-span-3' : 'block'}>
              {preview ? (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 max-w-3xl mx-auto">
                  ...
                  {form.image_url && <img src={form.image_url} className="w-full h-64 object-cover rounded-xl mb-8" alt="cover"/>}
                ...
                <div className="space-y-5">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="px-3 py-1 bg-yellow-50 text-yellow-700 border border-yellow-200 text-xs rounded-full font-medium">Brouillon</span>
                    </div>
                  ...
                  <div className="bg-red-50 border border-red-100 rounded-2xl p-5">
                    <div className="flex items-start gap-3">
                      <FileText className="w-5 h-5 text-[#d12127] flex-shrink-0 mt-0.5"/>
                      <div>
                        <h4 className="font-semibold text-red-900 text-sm mb-2">Conseils</h4>
                        <ul className="space-y-1.5 text-xs text-red-700">
                          {['Titre accrocheur et descriptif','Image de couverture (URL Unsplash recommandée)','Structurez avec des titres H2/H3','Extrait entre 100-200 caractères','Relisez avant publication'].map((tip,i) => (
